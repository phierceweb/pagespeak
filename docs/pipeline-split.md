...
- **gives pre-heading content a home.** Content before the document's first heading becomes a leading `Front Matter` section when it carries prose (title page, copyright, an abstract); a decoration-only preamble (a cover logo, empty-link debris) folds into the first section instead of standing as its own chunk. Inserted after parsing, so it never becomes anyone's parent and no other section's path changes; the usual `min_body_chars` gate still drops a trivial preamble;
- **keeps a contents list from parenting the document.** A section whose body is essentially a link list (≥2 link-only lines, ≥80% of the non-blank body) is navigation, not a container: its children re-attach to its own parent in document order, so real sections stay top-level instead of nesting under `table-of-contents/`. The contents section itself is kept. Logged as `split_promoted_nav_list_children`;
