...
| `--nested-split` / `--no-nested-split` | off | With `--split-sections`, nest section files in folders that mirror the heading hierarchy. Numbered sections use number prefixes (`1/1.4/1.4.1. Title.md`); semantic sections use sanitized heading titles (`Quick Start/Foot Switches (1).md`). |
