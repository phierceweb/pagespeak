...
| `pagespeak vision-audit <paths…> [--summary-only] [--strict]` | Flag likely-confabulated vision captions for human review: compares each figure's generated caption to the author's source alt text and flags a caption keeping none of the alt's subject words (a squirrel captioned as a lemur). Deterministic, $0, no LLM; only figures with a clear source alt are assessable, figures without alt are skipped. Exit 0 by default; `--strict` exits 1 on findings. See [audit.md](audit.md). |
...
Either way, when the target output dir holds a `.pagespeak-run.json` from a previous run, flags you don't pass default to that record's `resolved_flags` (explicit flags win; `--no-inherit` opts out) — see [caching.md](caching.md) § "Re-run flag inheritance".
...
| `--force-ocr` / `--no-force-ocr` | off | PDF only — force surya OCR even on text-bearing PDFs |
...
| `--split-sections` / `--no-split-sections` | off | Also write per-section files under `<output_dir>/sections/` plus an `INDEX.md` |
| `--nested-split` / `--no-nested-split` | off | With `--split-sections`, nest section files in folders that mirror the heading hierarchy. Numbered sections use number prefixes (`1/1-4/1-4-1-title.md`); semantic sections use their heading titles (`quick-start/foot-switches-1.md`). Every path component is slugified — see [docs/pipeline-split.md](pipeline-split.md). |
...
| `--split-max-level` | none | With `--split-sections`, cap section depth — headings deeper than this stay **inline** as content of their enclosing section. `--split-max-level 2` gives one file per H2 with subsections inline (textbook section-level chunks: one file per `## N.M` section and per unnumbered back-matter H2, `### `+ deeper inline). Pairs with `--split-min-level` for an exact band. Default: no cap. |
...
| `--inherit` / `--no-inherit` | on | When the output dir holds a `.pagespeak-run.json`, flags you don't pass default to that record's `resolved_flags` (explicit flags and an explicit `--preset` win; LLM/engine flags never inherit). `--no-inherit` ignores the record — bare defaults, the pre-0.6 behavior. See [docs/caching.md](caching.md) § "Re-run flag inheritance". |
| `--rerun-from <stage>` | none | Bust caches at this stage and downstream, then run. Deleted downstream outputs (`sections/`, `INDEX.md`) are rebuilt with the record-inherited flags (see `--inherit`). See [docs/caching.md](caching.md). |
...
Per-task **model + sampling kwargs** (model, max_tokens, temperature, etc.) live in `config/model_router.yaml` — not in env. The file is read by pf-core's model router (`pf_core.llm.router`); a copy ships **inside the package** as the default, so a `pip install`ed pagespeak has the tuned models out of the box. It is resolved (highest first) from the `MODEL_ROUTER_CONFIG` env var, then a `config/model_router.yaml` in the working directory, then the bundled default; an explicitly-set `MODEL_ROUTER_CONFIG` pointing at a missing or malformed file is a hard `ConfigurationError` (fail-fast, no silent model fallback). Edits hot-reload within `MODEL_ROUTER_RELOAD_SECONDS` (default 60) without a restart. The legacy `PAGESPEAK_VISION_MODEL` / `PAGESPEAK_NORMALIZE_HEADINGS_MODEL` env vars are no longer read, and a `PAGESPEAK_<TASK>_BACKEND` value that isn't one of the declared backends now falls back to the default (`claude_code`) instead of raising. Edit the YAML (or point `MODEL_ROUTER_CONFIG` at your own file) to change which model runs per task per backend; per-call CLI flags (`--vision-model`, `--normalize-headings-model`) still win as the highest-precedence override.
...
Stamps a YAML frontmatter block (`source_type` / `source_label` / `source_file` + `doc_title` + per-section locators) on the whole-doc markdown **and every section file**, so a downstream RAG DB combining several books, labs, and lectures can tag and locate each retrieved chunk by origin. The label auto-derives from the cleaned filename when `--source-label` is omitted; `source_type` is omitted from the block unless supplied (it's never guessed). `INDEX.md` is left untagged (it's navigation, not a chunk). Provenance is on under `--preset rag-default` (or the bare `--provenance` flag); without either, the **master doc** is byte-for-byte unchanged. Re-tag an already-converted doc at zero LLM cost with `--from split --preset rag-default` (re-uses the `visioned.md` checkpoint).

Independently of the provenance flags, **every section file always carries structural identity frontmatter**: `doc_id` (the conversion/out-dir name), `source_id` + `source_sha256` (a stable slug of the source filename and the SHA-256 of its exact bytes — which source work, which version; omitted when unrecoverable rather than guessed), `section_id` (the section's relative path — a stable join key), `parent_id` (nearest written ancestor's `section_id`), `section_title` / `section_path` / `section_number` / `heading_level`, `depth`, and `order` (1-based document order). Provenance source fields merge into the same block when enabled. This is what lets a consumer scope a query to one document (`doc_id`) or one source work across re-conversions (`source_id`), and walk parent/sibling sections without another retrieval round-trip. See [docs/pipeline-split.md](pipeline-split.md).
