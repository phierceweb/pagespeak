...
Independently of the provenance flags, **every section file always carries structural identity frontmatter**: `doc_id` (the conversion/out-dir name), `section_id` (the section's relative path — a stable join key), `parent_id` (nearest written ancestor's `section_id`), `section_title` / `section_path` / `section_number` / `heading_level`, `depth`, and `order` (1-based document order). Provenance source fields merge into the same block when enabled. This is what lets a consumer scope a query to one document (`doc_id`) or walk parent/sibling sections without another retrieval round-trip. See [docs/pipeline-split.md](pipeline-split.md).
